...
const Reviews = (props) => {
    ...
    async function fetchReviewsAPI() {
        const url = `https://api.themoviedb.org/3/${mediaType}/${mediaId}/reviews`;
        try {
  ...
  return (
    <div className='py-4'>
      <h1 className="text-3xl font-medium title-font text-gray-900 mb-12 text-center dark:text-white">Review</h1>
      {ReviewsData.results && ReviewsData.results.length > 0 && (
        <div className="container px-5 py-4 mx-auto max-h-96 overflow-auto">
          <div id="animation-carousel" data-carousel="static" className="relative xl:w-1/2 lg:w-3/4 w-full mx-auto text-center">
            {ReviewsData.results.map(({ content, author, created_at }, index) => (
              <div key={index} className='py-2'>
                ...
                <p className="text-gray-500">{new Date(created_at).toLocaleDateString()}</p>
              </div>
            ))}
          </div>
        </div>
